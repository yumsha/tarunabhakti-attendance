...
function exportTableExcel(students) {
  ...
  for (let R = range.s.r + 1; R <= range.e.r; ++R) {
    const nisnCell = XLSX.utils.encode_cell({ r: R, c: 0 }); // NISN
    const nipdCell = XLSX.utils.encode_cell({ r: R, c: 1 }); // NIPD
    const nikCell = XLSX.utils.encode_cell({ r: R, c: 2 });  // NIK
  ...
  const runUpdate = async (rowsToProcess, totalCount) => {
    ...
    try {
      ...
      if (r?.success && Array.isArray(r.data)) {
        r.data.forEach((s) => { siswaMap[String(s.nisn || s.NISN)] = s; });
      }
    } catch (_) { }
    ...
    rowsToProcess.forEach(({ row, idx }) => {
      ...
      const existingDateStr = existing.tgl_lahir ? existing.tgl_lahir.slice(0, 10) : (existing.tanggal_lahir ? existing.tanggal_lahir.slice(0, 10) : "");
...
// Add Siswa Modal — single-add satu siswa dengan form orang tua opsional (toggle)

function AddSiswaModal({ kelasList, onClose, onAdded }) {
  const emptyForm = {
    nisn: "",
    nipd: "",
    nik: "",
      ...
      if (!addOrtu && formData.orangtua_id.trim()) {
        try {
          ...
          if (ortuRes?.success && ortuRes.data) {
            ...
            console.error("Gagal get orang tua:", ortuRes);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">NIPD *</label>
                <input type="text" name="nipd" value={formData.nipd} onChange={handleChange} required
                  className="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono" />
              </div>
              {/* NIK */}
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Tempat Lahir *</label>
                <input type="text" name="tempat_lahir" value={formData.tempat_lahir} onChange={handleChange} required
                  className="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              {/* Tanggal Lahir */}
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Tanggal Lahir *</label>
                <input type="date" name="tgl_lahir" value={formData.tgl_lahir} onChange={handleChange} required
                  className="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              {/* Jenis Kelamin */}
              <div>
              ...
              <div className="p-4 space-y-4 border-t border-gray-200 bg-blue-50/30">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input type="text" name="nama_orangtua" value={ortuData.nama_orangtua} onChange={handleOrtuChange}
                      className="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  {/* NIK Ortu */}
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">NIK Orang Tua (16 digit) *</label>
                    <input type="text" name="NIK" value={ortuData.NIK} onChange={handleOrtuChange} maxLength={16}
                      className="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono" />
                  </div>
                  {/* No Telp Ortu */}
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">No. Telepon (08xx) *</label>
                    <input type="text" name="nomor_telepon" value={ortuData.nomor_telepon} onChange={handleOrtuChange}
                  ...
                  <div>
                    ...
                    <input type="text" name="pekerjaan" value={ortuData.pekerjaan} onChange={handleOrtuChange}
                      className="w-full text-xs px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  {/* Alamat */}
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-gray-700 mb-1">Alamat *</label>
                    <textarea name="alamat" value={ortuData.alamat} onChange={handleOrtuChange} rows={3}
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50/80">
                <tr>
                  {["Nama", "Kelas", "NIPD", "NISN", "NIK", "Tempat Lahir", "Jenis Kelamin", "Agama", "Nama Orang Tua", "Aksi"].map((h) => (
                ...
                {loading ? (
                  ...
                  currentPageData.map((s) => (
                    <tr key={s.id} className="hover:bg-blue-50/30 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600 font-mono">{s.NIPD || s.nipd}</td>
                      <td className="px-6 py-4 text-sm text-gray-600 font-mono">{s.NISN || s.nisn}</td>
                      <td className="px-6 py-4 text-sm text-gray-600 font-mono">{s.NIK || s.nik}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{s.tempat_lahir || "-"}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{s.jenis_kelamin || s.gender || "-"}</td>
