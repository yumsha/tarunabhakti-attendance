...
export default function UserFormModal({ isOpen, onClose, onSubmit, editUser }) {
  ...
  const getInitialRole = (userData) => {
    const roles = resolveRoles(userData);
    if (roles.includes("ADMIN")) return "ADMIN";
    if (roles.includes("WALAS")) return "WALAS";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (finalRole === "GURU" || finalRole === "WALAS") {
      ...
      const guruData =
        finalRole === "GURU" || finalRole === "WALAS"
          ? {
  ...
  const showGuruFields = form.role === "GURU" || form.role === "WALAS";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-3 gap-2">
              {ROLE_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`flex items-center justify-center py-2.5 rounded-xl border-2 text-sm font-medium transition ${
                    form.role === opt.value
          ...
          {/* Data Guru (untuk role GURU dan WALAS) */}
          {showGuruFields && (
